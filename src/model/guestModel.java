...
public class guestModel extends userModel{
	...
	public guestModel(String user_id, String email, String name, String password, String role) {
		super(user_id, email, name, password, role);
		this.con = makeConnection.getInstance();
	}
	
	public List<eventModel> seeInvitationGuest(){
		String query = "SELECT e.event_id, e.event_name, e.event_date, e.event_location, e.event_description "
				...
				+ "WHERE i.invitation_status = 'pending' and i.invitation_role = 'Guest'";
	...
	public List<eventModel> seeAcceptedInv() {
		String query = "SELECT e.event_id, e.event_name, e.event_date, e.event_location, e.event_description "
				+ "FROM invitation i "
				+ "JOIN eventmodel e ON i.event_id = e.event_id "
				+ "WHERE i.invitation_status = 'accept' and i.invitation_role = 'Guest'";
		List<eventModel> events = new ArrayList<>();
		PreparedStatement ps = con.prepareStatement(query);
		try {
			ResultSet rs = ps.executeQuery();
